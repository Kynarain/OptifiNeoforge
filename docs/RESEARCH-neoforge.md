...
- **1.20.2 / 1.20.4：A（原样可用）**。Mojang official 运行时（不是 SRG，OptiFine 的 `patch/srg` 树仍匹配，因为类名在 SRG/official 下一致），`mods.toml` 仍被接受，`loaderVersion` 有 `javafml=42` 后门，无 OptiFine 拒收检查。
